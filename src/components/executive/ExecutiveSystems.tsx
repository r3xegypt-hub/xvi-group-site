...
export function ArchitectureGrid({ items }: { items: Item[] }) {
  return <div className="grid overflow-hidden rounded-[30px] border border-[color:var(--color-xvi-line)] bg-white/45 sm:grid-cols-2 lg:grid-cols-3">{items.map((item, index) => <div key={item.label} className="group min-h-52 border-b border-[color:var(--color-xvi-line)] p-7 last:border-b-0 sm:[&:nth-last-child(2)]:border-b-0 lg:[&:nth-last-child(-n+3)]:border-b-0 lg:border-s"><div className="flex items-center justify-between"><span className="text-xs tracking-[.2em] text-[color:var(--color-xvi-bronze)]">0{index + 1}</span><span className="h-2 w-2 rounded-full bg-[color:var(--color-xvi-bronze)] shadow-[0_0_0_6px_rgba(201,169,110,.1)] transition-transform group-hover:scale-125" /></div><h3 className="mt-12 text-2xl font-semibold tracking-[-.045em]">{item.label}</h3><p className="mt-3 max-w-xs text-sm leading-7 text-[color:var(--color-xvi-ink-soft)]">{item.detail}</p></div>)}</div>
}
