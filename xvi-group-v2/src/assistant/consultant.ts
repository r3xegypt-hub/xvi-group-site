...
const GREETING_AR: RegExp[] = [
  new RegExp(`^(هاي|اهلا|أهلا|أهلاً|اهلاً|السلام عليكم|السلام|الو|ألو|هلا|هيلو|مرحبا|مرحباً|صباح الخير|مساء الخير|مساء النور|ازيك|إزيك|ازيكم|عامل ايه|كيفك|كيف حالك|شلونك|شلونكم|شخباركم|كيف الصحه|هلا وغلا|هلا بيك|يسعد اوقاتك|يسعد أوقاتك)${AR_END}`),
...
const PROJECT_AR: RegExp[] = [
  /(نحتاج|أحتاج|احتاج|أريد|اريد|نريد|أرغب|ارغب|عندي|عندنا|لدينا|نبي|نبغى|بغيت|عايز|عاوز|عايزين|عاوزين|بدنا|ودي|حاجين)\s/,
  new RegExp(`(عندي|عندنا|لدينا|معنا)\\s+(مشروع|فكرة|منتج|نظام|تطبيق|موقع)${AR_END}`),
  /(ساعدني|ساعدوني|ساعدنا|تقدروا تساعدونا|ممكن تساعدوني)\s+(في|على)?\s*(بناء|تطوير|إنشاء|انشاء|تصميم|إطلاق|عمل|تسوية)/,
];
